...
import { getPlantEcologyDefinitions, type PlantGridCell } from "../../../lib/simulation/plant-engine";
...
import { PlantMapClient } from "./plant-map-client";

export const dynamic = "force-dynamic";

function UnavailablePlantSnapshot() {
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <section className="mt-8 border border-white/10 bg-black/20 p-6 text-sm text-stone-300">
        No persisted plant ecology snapshot is available yet. Start the simulation worker or run npm run sim:step.
      </section>
      <Link
...
export default async function WorldsPlantsPage() {
  ...
  const plantSummary = snapshot?.plantSummary ?? null;

  return (
    <main className="min-h-screen bg-dawn-coal px-6 py-10 text-stone-100 sm:px-10 lg:px-12">
      {!activeWorld || !snapshot || !plantSummary ? (
        <UnavailablePlantSnapshot />
      ) : (
        <>
          ...
          <PlantMapClient
            cells={snapshot.cells as readonly PlantGridCell[]}
