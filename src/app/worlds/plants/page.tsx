...
import { getPlantEcologyDefinitions, getPlantEcologyState } from "../../../lib/simulation/plant-engine";
...
export default async function WorldsPlantsPage() {
  ...
  const plantState = activeWorld?.seed?.trim() ? getPlantEcologyState(activeWorld, grid) : null;

  return (
    <main className="min-h-screen bg-dawn-coal px-6 py-10 text-stone-100 sm:px-10 lg:px-12">
      {!activeWorld || !planet || !plantState ? (
        <div className="mx-auto max-w-4xl">
          ...
          <section className="mt-8 border border-white/10 bg-black/20 p-6 text-sm text-stone-300">
            A seeded world is required before plant ecology cells can be generated.
          </section>
          <Link
